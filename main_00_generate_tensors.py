"""
main_00_generate_tensors.py

Main script for a DFTS experiment.

Splits a given Keras model at the chosen split layer.
For the device sub-model, generates the output tensors.

From terminal:
python main_00_generate_tensors.py -p params_00_generate_tensors.yml
...
from spimulation.run_experiment_00_device_tensor import runSimulation
...
def userInterface():
    ...
    if os.path.isfile(modelPath):
        model = modelPath
        print('The tf.keras model used is')
        print(model)
    ...
    runSimulation(model, splitLayer, task, modelDict, simDir, customObjects, evaluator)


# ---------------------------------------------------------------------------- #
if __name__ == "__main__":
    # Verify version of Tensorflow
